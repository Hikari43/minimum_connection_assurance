...
def summary(model, scores, flops, prunable):
    ...
    for name, module in model.named_modules():
        for pname, param in module.named_parameters(recurse=False):
            ...
            if pruned:
                density = getattr(module, pname+'_mask').detach().cpu().numpy().mean()
            ...
            rows.append([name, pname, density, np.prod(shape), shape, flop,
                         score_mean, score_var, score_sum, 
                         score_abs_mean, score_abs_var, score_abs_sum, 
                         pruned])

    columns = ['module', 'param', 'density', 'size', 'shape', 'flops', 'score mean', 'score variance', 
