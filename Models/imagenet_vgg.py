...
class VGG(nn.Module):
    ...
    def set_num_out_channel(self, list_E, V_out_select):
        c = self.num_classes
        for i, layer in enumerate(reversed(self.classifier)):
            if hasattr(layer, 'get_num_out_channel'):
                if i == (len(self.classifier) - 1):
                    c = layer.get_num_out_channel(c, list_E, V_out_select, (7, 7))
                else:
    ...
    def set_scores(self, x, scores, shuffle_V_out=False, set_only_V_in=False):
        ...
        for i, layer in enumerate(self.classifier):
            is_last = True if i == (n_classifiers - 1) else False
            if hasattr(layer, 'forward_set_scores'):
                x = layer.forward_set_scores(x=x, scores=scores, shuffle_V_out=is_last, set_only_V_in=set_only_V_in)
                x = 1.0 * (x != 0)
            elif isinstance(layer, nn.MaxPool2d):
                x = layer(x)
                x = 1.0 * (x != 0)
    ...
    def set_effective_sparsity(self, is_forward, x=None):
        ...
        if is_forward:
            ...
            for i, layer in enumerate(reversed(self.classifier)):
                if hasattr(layer, 'modify_effective_sparsity'):
                    if i == (len(self.classifier) - 1):
                        connected_channels, _ = layer.modify_effective_sparsity(connected_channels=connected_channels, is_forward=is_forward, x=None, pseudo_kernel_size=(7, 7))
                    else:
                        connected_channels, _ = layer.modify_effective_sparsity(connected_channels=connected_channels, is_forward=is_forward, x=None)
                    print(f'{connected_channels=}')
            for layer in reversed(self.features):
